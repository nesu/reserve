...
{
    public static class DeleteServiceAssignment
    {
        [PublicAPI]
        ...
        public class RequestValidator : AbstractValidator<RequestEnvelope>
        {
            public RequestValidator(ValidationHelpers vh)
            {
                ...
                RuleFor(it => it.Id)
                    .Must(vh.ServiceAssigneeExists).WithMessage("Assignment does not exist");
            }
        }
        ...
        public class Handler : IRequestHandler<RequestEnvelope, Unit>
        {
            ...
            {
                var service = await _context.ServiceAssignees
                    .Include(it => it.Assignee)
                    .ThenInclude(it => it.AssignedReservations)
                    .FirstOrDefaultAsync(it => it.Id == request.Id, ct);

                if (service != null)
                {
                    _context.ServiceAssignees.Remove(service);
