<template>
    <v-dialog
        v-model="active"
        fullscreen persistent>
        <v-sheet
            height="100%">
            <v-container
                class="narrowed fill-height">
                <v-card
                    class="d-flex flex-column"
                    height="100%" width="100%"
                    flat tile>
                    <div
                        class="flex-shrink-1">
                        <v-toolbar
                            flat
                            fixed
                            height="76"
                            class="pl-4"
                            color="transparent"
                            width="100%">
                            <v-toolbar-title>
                                Create navigator element
                            </v-toolbar-title>
                            <v-spacer/>
                            <v-btn
                                icon class="mr-2"
                                @click="close">
                                <v-icon>mdi-close</v-icon>
                            </v-btn>
                        </v-toolbar>
                    </div>
                    <v-divider class="mx-4"/>
                    <div
                        class="pt-6 flex-grow-1">
                        <v-card-text>
                            <v-text-field
                                outlined
                                label="Title"
                                v-model="navigator_element_request.title"/>
                            <v-text-field
                                outlined
                                label="Icon identifier"
                                hide-details="auto"
                                v-model="navigator_element_request.mdi_icon"/>
                            <span
                                class="overline">
                                <a target="_blank" href="https://materialdesignicons.com/">
                                    Supported icons
                                </a>
                            </span>
                            <v-text-field
                                outlined class="mt-4"
                                v-model="navigator_element_request.redirect_to"
                                hide-details label="Destination URL"/>
                            <span
                                class="overline">
                                <v-dialog
                                    width="500"
                                    v-model="selecting_page">
                                    <template v-slot:activator="{ on }">
                                        <a v-on="on">
                                            Select page
                                        </a>
                                    </template>
                                    <v-card
                                        flat tile>
                                        <v-card-title>
                                            Redirect to...
                                        </v-card-title>
                                        <v-card-text
                                            v-if="pages.length === 0"
                                            class="text-center">
                                            Currently there are no pages...
                                        </v-card-text>
                                        <v-card-text
                                            v-else>
                                            <v-select
                                                outlined
                                                label="Select page"
                                                :items="pages"
                                                v-model="navigator_element_request.redirect_to"
                                                :item-value="(it) => `/pages/${it.identifier}`"
                                                @change="selecting_page = false"
                                                item-text="title"/>
                                        </v-card-text>
                                    </v-card>
                                </v-dialog>
                            </span>
                            <v-text-field
                                outlined class="mt-4"
                                label="Order of appearance"
                                v-model="navigator_element_request.order"/>
                        </v-card-text>
                        <v-card-text>
                            <v-checkbox
                                label="Mobile view"
                                v-model="navigator_element_request.mobile_view"
                                hide-details/>
                            <v-checkbox
                                label="Visible only for guests"
                                v-model="navigator_element_request.guest_only"
                                hide-details/>
                            <v-checkbox
                                v-model="navigator_element_request.authenticated_only"
                                label="Visible only for logged in users"/>
                        </v-card-text>
                        <v-card-text>
                            <v-select
                                label="Required role(s)"
                                messages="Applied only if element requires authentication"
                                outlined color="primary"
                                :disabled="!navigator_element_request.authenticated_only"
                                :items="$readable_role_array"
                                v-model="navigator_element_request.permitted_roles"
                                item-text="role"
                                item-value="value"
                                chips multiple/>
                        </v-card-text>
                    </div>
                    <v-divider class="mx-4"/>
                    <v-card-actions>
                        <v-spacer/>
                        <v-btn
                            @click.stop="create_navigator"
                            color="primary">
                            Submit
                        </v-btn>
                    </v-card-actions>
                </v-card>
            </v-container>
        </v-sheet>
    </v-dialog>
</template>
<script lang="ts">

    import { Component, Property, Watch } from "~/commons";
    import { create_navigator_element, get_created_pages } from "~/commons/requests/layout";
    import CreateNavigatorElementRequest from "~/commons/requests/models/create-navigator-element-request";
    import NavigatorElementResource from "~/commons/resources/navigator-element-resource";
    import Page from "~/commons/resources/page";
    import { RoleType } from "~/commons/resources/role-type";
    import { mixins } from "../../commons";
    import InputValidators from "../../commons/mixins/input-validators";
    import PageBase from "../../commons/mixins/page-base";

    @Component
    export default class NavigatorCreatorComponent extends mixins(PageBase, InputValidators)
    {
        pages: Page[] = [];
        selecting_page: boolean = false;

        @Property({
            type: Boolean,
            required: true
        })
        active!: boolean;

        mounted()
        {
            get_created_pages()
                .then((response) => {
                    this.pages = response;
                });
        }

        navigator_element_request: CreateNavigatorElementRequest = {
            order: 0,
            title: "",
            mdi_icon: "",
            redirect_to: "",
            mobile_view: false,
            guest_only: false,
            authenticated_only: false,
            permitted_roles: [],
            properties: {
                text: true
            }
        };

        create_navigator()
        {
            create_navigator_element(this.navigator_element_request)
                .then(() =>
                {
                    this.$notify_message("Navigator element created");
                    this.$emit("updated:navigators");
                    this.close();
                });
        }

        close() {
            this.$emit("update:active", false);
        }

        @Watch("navigator_element_request.guest_only")
        guest_only_watcher(value: boolean)
        {
            if (value) {
                this.navigator_element_request.permitted_roles = [];
                this.navigator_element_request.authenticated_only = false;
            }
        }

        @Watch("navigator_element_request.authenticated_only")
        authenticated_only_watcher(value: boolean)
        {
            if (value) {
                this.navigator_element_request.guest_only = false;
            }
        }
    }


</script>
